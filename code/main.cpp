 ...
 * - 6) 其他
实验结论：
本次期中大作业，我基于提供的框架代码，成功实现了一个功能相对完整的俄罗斯方块游戏。我主要完成的工作包括：
1.实现了所有7种方块, 定义了包括 I, O, S, Z, J, L, T 在内的所有标准方块形状及其旋转状态的坐标数据。

2.完成了核心游戏逻辑：
实现了方块的随机生成（包括形状和颜色）与自动下落（基于时间）。
完善了碰撞检测逻辑，确保方块能在边界内移动，并能正确堆叠在其他已固定方块上。实现了方块的固定 (setTile) 和行消除 (checkFull, eliminateRow) 机制，包括消除后上方方块的下落。

3.实现了完整的游戏流程控制：
引入了游戏状态机 (GameState)，管理开始界面、游戏进行、暂停和游戏结束四个状态。实现了相应的键盘交互逻辑（Enter 开始，P 暂停/恢复，R 重新开始，Q/ESC 退出）。

4.添加了数据持久化:
引入了 nlohmann/json 库，并创建了 loadAndSave 模块。
实现了在游戏开始前获取用户名，并在游戏结束后自动保存包含用户名、得分、时长和时间的游戏记录到 JSON 文件。
实现了在游戏启动时加载并打印历史游戏记录的功能。

5.解决了跨平台和环境问题:
在 Windows 环境下配置了 vcpkg 来管理 glfw, glm, nlohmann-json 等依赖。
解决了跨平台 API 兼容性问题以及 MSVC 编译错误。
 */
...
void gameover()
{
	...
	std::cout << "用户名: " << gameRecord.getUsername() << std::endl;
	std::cout << "时间: " << gameRecord.getDateTime() << std::endl;
	std::cout << "用时: " << gameRecord.getDuration() << std::endl;
	std::cout << "最终得分: " << gameRecord.getScore() << std::endl;
	std::cout << "-------------------------------------------------------------" << std::endl;
	score = 0;
	gameRecord.clear();
...
void key_callback(GLFWwindow *window, int key, int scancode, int action, int mode)
{
	...
	if (key == GLFW_KEY_ESCAPE || key == GLFW_KEY_Q)
	{
		...
		{
			// 如果游戏是进行状态或者停止状态, 才有必要切换成gameover也就是结束状态
			gameover();
		}
...
int main(int argc, char **argv)
{
	...
	// scoreBoardWidth = 150;							  // 假设给计分板留 150 像素
